...
const MALFORMED_TOOL_CALL_CONTINUATION_PROMPT =
  "The previous assistant turn attempted a tool call whose JSON arguments were invalid, so the call was rejected before execution. Continue from the existing transcript without repeating completed work. If another tool call is needed, reconstruct its arguments as strict valid JSON instead of reusing the malformed arguments; otherwise, return the final answer.";
...
> {
  ...
  if (
    ...
    sessionPromptState.activateInternalPrompt(MALFORMED_TOOL_CALL_CONTINUATION_PROMPT);
    log.warn(
