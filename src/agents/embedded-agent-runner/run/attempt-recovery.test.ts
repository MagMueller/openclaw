...
describe("recoverEmbeddedRunAttempt", () => {
  ...
  it("continues a malformed tool call from settled results without replaying completed tools", async () => {
    ...
    expect(fixture.activateInternalPrompt).toHaveBeenCalledExactlyOnceWith(
      expect.stringContaining("JSON arguments were invalid"),
    );
    expect(fixture.continueFromCurrentTranscript).not.toHaveBeenCalled();
    ...
    expect(await fixture.recover()).toEqual({ action: "proceed" });
    expect(fixture.activateInternalPrompt).toHaveBeenCalledTimes(2);
  });
